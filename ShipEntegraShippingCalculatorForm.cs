...
internal sealed class ShipEntegraShippingCalculatorForm : Form
{
    ...
    private Control BuildTokenBar()
    {
        ...
        _txtBearerToken.PlaceholderText = "app.shipentegra.com Bearer tokeni (veya sağdaki Otomatik / Tarayıcı butonları ile çekin)";
        ...
        _btnAutoLogin.FlatStyle = FlatStyle.Flat;
    ...
    private async Task TriggerAutoSessionRefreshAsync(bool forceDialog, bool directBrowser = false)
    {
        ...
        else if (forceDialog || string.IsNullOrWhiteSpace(email) || string.IsNullOrWhiteSpace(pass))
        {
            ...
            {
                _settings.SavedEmail = email;
                _settings.EncryptedPassword = ShippingCredentialEncryptor.Encrypt(pass);
                _settings.AutoRefreshEnabled = dlg.AutoRefresh;
        ...
        try
        {
            string? freshToken = await _sessionManager.RefreshShipEntegraTokenAsync(
                ...
                msg =>
                {
                    if (InvokeRequired) Invoke(() => _lblSubtitle.Text = msg);
                    else _lblSubtitle.Text = msg;
                });

            if (!string.IsNullOrWhiteSpace(freshToken))
            {
                _txtBearerToken.Text = freshToken;
                _settings = ShipEntegraSettingsStore.Load();
                MessageBox.Show(
                    "ShipEntegra canlı oturumu başarıyla açıldı ve yeni token alındı!",
                    "Oturum Açıldı",
            ...
            {
                MessageBox.Show(
                    "Otomatik oturum açılamadı. Dilerseniz 'Tarayıcıda Aç' diyerek bir kez manuel giriş yapabilirsiniz.",
                    "Bilgi",
                    MessageBoxButtons.OK,
